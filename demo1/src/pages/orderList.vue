...
export default {
  ...
  computed: {
    tableData () {
       return this.$store.getters.getOrderList //第一步
    }
  },
  methods: {
    productChange (obj) {
    ...
    getTableData () {
      ...
      .then((res) => {
        this.tableData = JSON.parse(res.bodyText).list
      }, (err) => {
  ...
  mounted () {
    //this.getTableData ()
    this.$store.dispatch('fetchOrderList') //第二步 dispatch这个action,最后action会commit一个mutation,最终mutation会把state赋值给orderList
